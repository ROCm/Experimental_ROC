## Tools to Install ROCm 1.9.1 on Ubuntu 16.04 From Source
The scripts in this directory will download, build, and install ROCm 1.9.1 from source on Ubuntu 16.04.5 LTS.
...
```bash
./00_prepare_system_ubuntu_16.04.sh -r
```
...
```bash
./03_install_rocm_libraries_ubuntu_16.04.sh -l -i ${rocm_installation_directory} -o {rocm_installation_directory}
```
...
This section assumes that you have a ROCm 1.9.1 installation in the default `/opt/rocm/` location, though the directions would also work if you have ROCm installed into some other custom location.
