## Tools to Install ROCm 2.0.0 on CentOS 7.5 From Source
The scripts in this directory will download, build, and install ROCm 2.0.0 from source on CentOS 7.5.
...
```bash
./01_install_rocm_centos_7.5.sh -p {directory_to_store_packages} -o {rocm_installation_directory}
```
...
```bash
./03_install_rocm_libraries_centos_7.5.sh -p {directory_to_store_packages} -o {rocm_installation_directory}
```
