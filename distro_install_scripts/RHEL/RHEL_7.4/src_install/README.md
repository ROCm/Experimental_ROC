## Tools to Install ROCm 2.0.0 on RHEL 7.4 From Source
The scripts in this directory will download, build, and install ROCm 2.0.0 from source on RHEL 7.4.
...
```bash
./01_install_rocm_rhel_7.4.sh -p {directory_to_store_packages} -o {rocm_installation_directory}
```
...
```bash
./03_install_rocm_libraries_rhel_7.4.sh -p {directory_to_store_packages} -o {rocm_installation_directory}
```
